...
window.AD = (function(){
  ...
  var BACKDROPS = ['a subtly textured wall in soft, even shadow','bookshelves falling into soft shadow','a colleague softly out of focus across the room','a corridor or doorway in medium-dark shade','a wood-panelled wall in warm, dim shade','a plant-filled corner in shadow','a room that falls off into medium-dark shadow','sunlight on a far wall, cast through blinds or leaves from a window out of frame'];
  ...
  var PSHOTS = {
    'Over the shoulder': 'Shot on 50mm at f/2.8 at seated eye level, just behind and to one side, over their shoulder, as close as a colleague sitting beside them. The screen face fills the middle of the frame, with the hands and a soft edge of their shoulder and profile in the foreground.',
  ...
  var PCAMS = {
    'Over the shoulder': 'Camera: at seated eye level, just behind the professional and to one side, looking over their shoulder at the front of the open laptop screen, the same view they have.',
  ...
  var PSHOT_S = {'Over the shoulder':'50mm over the shoulder at seated eye level, screen face centered, hands and profile edge in foreground','Point of view':'35mm first-person view down onto the screen, hands at the keyboard','Lifestyle':'35mm three-quarter view, professional beside a side-on laptop, screen face toward camera'};
  ...
  function short(facet,o){
    if(facet==='Professional') return 'Candid editorial photo, '+o.subject+' '+o.action+' in '+o.setting+', face turned into '+LIGHT_S+', light spotlighting them in their element, '+NOSUN_S+', seated eye level, camera in the room as close as a colleague, blurred foreground, classic, timeless workwear in quiet neutrals with 1 small ambient accent in deep teal #004A66, navy #002533, wine #570E2E or dark wine #410A23, under 10% of the frame, never bright, editorial story about the new way professionals present, candid and unposed, face in sharp focus, crisp skin highlights, open facial shadows with detail, visible pores and fine hair, room falling into medium to medium-dark shadow, no retouching or smoothing, '+TONE_S+', '+LOOK_S+', no writing or words anywhere --ar '+AR[facet]+' --style raw';
    if(facet==='Product') return 'Editorial product photo, screen is the hero, '+(PSHOT_S[o.shotName]||PSHOT_S['Over the shoulder'])+', '+o.subject+' at a laptop in '+o.setting+', front of the screen facing camera showing a plain white canvas, back of the lid never visible, lit by '+LIGHT_S+', '+NOSUN_S+', silver, grey or black laptop, '+TONE_S+', '+LOOK_S+' --ar '+AR[facet]+' --style raw';
  ...
  function brief(facet,o,shotName){
    ...
    if(facet==='Professional'){l.push('Action: candid, mid-task, actively working; never posed or idle','Props: '+o.props,'Shot: '+shotName+'. '+o.shot,'Camera: seated eye level, soft foreground element','Presence: in the room, as close as a colleague would sit, never a distant long-lens view','Gaze: '+o.gaze,'Wardrobe: '+o.wardrobe+', quiet neutrals','Accent: 1 ambient hint in deep teal #004A66, navy #002533, wine #570E2E or dark wine #410A23, no more than 10% of the frame, never stealing focus','Devices: laptop secondary, screen unreadable, standard silver, grey or black');}
  ...
  var SHOT_S={'Close-up':'85mm close-up, just below eye level, easy smile, bright space blurred behind','Medium shot':'50mm, waist-up, laptop in frame or implied','Wide shot':'35mm from across the table or room, near enough to join them'};
  ...
  function build0(facet,sel,tool){
    ...
    var o={
      ...
      action: ACTION_G,
      traits: (function(){var t=pick(TRAITS.filter(function(x){return !(x[1]==='old'&&sel.age==='20s')}),2);LV.push(t[0][0],t[1][0]);return 'They have '+t[0][0]+' and '+t[1][0]+'.'})(),
      props: (function(){var p=pick(PROPS,2);LV.push.apply(LV,p);return p.join(' and ')})(),
    ...
    ['subject','backdrop','setting','accent','accentItem','copy','wardrobe','shot','pcam','layout','ground'].forEach(function(k){if(o[k])LV.push(o[k])});
  ...
  var ITEMS = [
    ...
    {src:'deck-grid-wall.png',facet:PZ,cap:'A deck, as a tilted grid',sel:{layout:'Grid',ground:'Dark'}},
